...
\begin{figure}[h]
	\centering
	\includegraphics[width=14cm]{bilder/abbildung_webserver_webclient}
...
Der Pfad \glqq / \grqq{} wird durch den Browser des Clients über die Eingabe des Domainnamens aufgerufen.

Der Pfad \glqq /api/sessions/:id \grqq{} wird über einen AJAX-Request aufgerufen, wenn auf der Startseite eine Session-ID 
eingeben wird und der Benutzer den Knopf \glqq Senden \grqq{} drückt.

Die Pfade \glqq /api/thumbnails/:id \grqq{} und \glqq /api/images/:id \grqq{} werden in der Datei index.html als Atrribute des Tags \textit{\textless img\textgreater} verwendet.

\subsection{Realisierung}
Der Webserver ist in JavaScript auf der Node.js-Plattform implementiert. Zur vereinfachten Installation und bequemeren Handhabung von Anfragen an den Server wird das Framework \textit{Express.js} verwendet. %\clearpage
...
In der Datei \textit{app.js} sind die Befehle zur Erzeugung des Web-Servers enthalten . Weiter werden in dieser Datei die übrigen Dateien zur Konfiguration der Datenbank und der Routen importiert. In Listing \ref{list_server} ist die Datei auszugsweise vorgestellt.

\begin{lstlisting}[caption={Auszug aus app.js}, label=list_server]
var express	= require("express"); // express importieren
var app		= express(); // Anwendung / Web-Server initialisieren
var indexRoutes = require("'./routes/index"); // Routen importieren
var apiRoutes = require("./routes/apiRoutes"); // Routen importieren

app.use("/", routes); // Routen verwenden für Anfragen an den Server
app.listen(60127); // Web-Server starten, auf Anfragen an Port 60127 hören
\end{lstlisting}
...
Die einzelnen Endpunkte der Schnittstelle sind wie in Listing \ref{list_api} implementiert:
...
Die erste Route in den Zeilen 2-4 reagiert auf die Anfrage an den Pfad \glqq / \grqq{}.
...
\glqq /api/sessions/:id \grqq{}, die Funktionsweise ist jedoch identisch zur Anfragen 
an \glqq /api/images/:id \grqq{} und \glqq /api/thumbnails/:id grqq{}.
